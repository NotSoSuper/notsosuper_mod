...
function notsosuper_CMD_DOWN(ply,cmd,args)
	...
	function qry_select_review:onSuccess( qry_data_review )
		if (qry_data_review[1]) then
			...
			local qry_insert_review = rip.db:query("INSERT INTO `pg_reputation` (`to_steamid`, `from_steamid`, `type`, `note`) VALUES ('"..notsosuper_VAR_STEAMID.."', '"..NOTSOSUPER_VAR_ADMINID.."', '0', "..sql.SQLStr(NIGGER_VAR_REASON)..")")
...
concommand.Add("notsosuper_down", notsosuper_CMD_DOWN)
lechat.AddCmd("/down ", nil, notsosuper_CMD_DOWN)
